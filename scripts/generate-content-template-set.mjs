...
function scene(config){
  ...
  rect(700,28,520,28,{backgroundColor:"#ffffff",strokeColor:"#bbc3c9",strokeWidth:1});text(858,34,"curiositypedia.app/journey/7",10,{strokeColor:"#667085"});
  text(48,88,"CURIOSITYPEDIA",15);text(810,90,"Journey",11);text(892,90,"Map",11,{strokeColor:"#667085"});text(1767,90,"1 turn · 4 sources",10,{strokeColor:"#667085"});ellipse(1870,90,12,12,{backgroundColor:"#81d88d",strokeColor:"#2f7d3a",strokeWidth:1});
...
for(const t of templates){const E=scene(t);const drawing={type:"excalidraw",version:2,source:"https://excalidraw.com",elements:E,appState:{gridSize:null,viewBackgroundColor:"#f7f5ef",currentItemFontFamily:5},files:{}};const stem=`design/content-templates/curiositypedia-content-${t.slug}`;fs.writeFileSync(`${stem}.excalidraw`,`${JSON.stringify(drawing,null,2)}\n`);const svg=svgFor(E);fs.writeFileSync(`${stem}.svg`,svg);await sharp(Buffer.from(svg)).png().toFile(`${stem}.png`);pngs.push(`${stem}.png`);console.log(`Created ${stem}.{excalidraw,svg,png}`)}
const thumbs=await Promise.all(pngs.map(p=>sharp(p).flatten({background:"#f7f5ef"}).resize(960,540).png().toBuffer()));await sharp({create:{width:1920,height:1080,channels:4,background:"#ece9e1"}}).composite(thumbs.map((input,i)=>({input,left:(i%2)*960,top:Math.floor(i/2)*540}))).flatten({background:"#ece9e1"}).png().toFile("design/content-templates/curiositypedia-content-template-contact-sheet.png");console.log("Created contact sheet");
